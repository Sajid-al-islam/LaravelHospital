<!DOCTYPE html>
<html class="no-js" lang="zxx">
    <!-- Mirrored from preview.colorlib.com/theme/docmed/ by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 20 Sep 2021 16:38:30 GMT -->
    <head>
        <meta charset="utf-8" />
        <meta http-equiv="x-ua-compatible" content="ie=edge" />
        <title>Docmed</title>
        <meta name="description" content="" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />

        <link rel="shortcut icon" type="image/x-icon" href="{{ asset('/img/xfavicon.png.pagespeed.ic.xjuSQ5i6l5.png') }}" />

        <link rel="stylesheet" href="{{asset('frontend/css/bootstrap.min.css%2bowl.carousel.min.css%2bmagnific-popup.css%2bfont-awesome.min.css%2bthemify-icons.css%2bnice-select.css%2bflaticon.css%2bgijgo.css%2banimat')}}" />
        <link rel="stylesheet" href="{{ asset('frontend/css/A.style.css.pagespeed.cf.hkYgbokBBp.css') }}" />
        @yield('links')
    </head>
    <body>

        @yield('content')

        @yield('script')
        <script src="{{ asset('frontend/js/vendor/modernizr-3.5.0.min.js') }}"></script>
        <script src="{{ asset('frontend/js/vendor/jquery-1.12.4.min.js') }}"></script>
        <script src="{{ asset('frontend/js/popper.min.js%2bbootstrap.min.js.pagespeed.jc.HYo1xDIROu.js') }}"></script>

        <script>eval(mod_pagespeed_Wg1ghn079g);</script>
        <script>eval(mod_pagespeed_l4j4h98Fuo);</script>
        
        <script src="{{ asset('frontend/js/owl.carousel.min.js%2bisotope.pkgd.min.js%2bajax-form.js%2bwaypoints.min.js%2bjquery.counterup.min.js.pagespeed.jc.fPKK8bVy7C.js') }}"></script>

        <script>eval(mod_pagespeed_HnhwjW_InZ);</script>
        <script>eval(mod_pagespeed_zal2vTGUrb);</script>
        <script>eval(mod_pagespeed_vXjTGNjwqZ);</script>
        <script>eval(mod_pagespeed_vlx8QI$ceE);</script>
        <script>eval(mod_pagespeed_3XPc$tpE8P);</script>
        
        <script src="{{ asset('frontend/js/imagesloaded.pkgd.min.js%2bscrollIt.js%2bjquery.scrollUp.min.js%2bwow.min.js%2bnice-select.min.js%2bjquery.slicknav.min.js%2bjquery.magnific-popup.min.js%2bp') }}"></script>

        <script>eval(mod_pagespeed__gK2jkRKUU);</script>
        <script>eval(mod_pagespeed_O4kg_ZyOB_);</script>
        <script>eval(mod_pagespeed_7psiNvlkIO);</script>
        <script>eval(mod_pagespeed_R7vp8C7fLA);</script>
        <script>eval(mod_pagespeed_BbLcuJjEHi);</script>
        <script>eval(mod_pagespeed_Di$K7wkOUE);</script>
        <script>eval(mod_pagespeed_0YXLPGuNQ2);</script>
        <script>eval(mod_pagespeed_B$0jefONcZ);</script>
        
        <script src="{{ asset('frontend/js/gijgo.min.js') }}"></script>

        <script src="{{ asset('frontend/js/contact.js%2bjquery.ajaxchimp.min.js%2bjquery.form.js%2bjquery.validate.min.js%2bmail-script.js%2bmain.js.pagespeed.jc.kQcYzjV8cs.js') }}"></script>

        <script>eval(mod_pagespeed_DuSzzTDZNK);</script>
        <script>eval(mod_pagespeed_MrfmLjxfTZ);</script>
        <script>eval(mod_pagespeed_in_if2LqV5);</script>
        <script>eval(mod_pagespeed_t$$qqw5FXj);</script>
        <script>eval(mod_pagespeed_EphGjvx2yw);</script>
        <script>eval(mod_pagespeed_K$dCl48UyR);</script>
        
        <script>
            $('#datepicker').datepicker({
            iconsLibrary: 'fontawesome',
            icons: {
            rightIcon: '<span class="fa fa-caret-down"></span>'
            }
            });
            $('#datepicker2').datepicker({
            iconsLibrary: 'fontawesome',
            icons: {
            rightIcon: '<span class="fa fa-caret-down"></span>'
            }
            });
            $(document).ready(function() {
            $('.js-example-basic-multiple').select2();
            });
        </script>

        <script async src="https://www.googletagmanager.com/gtag/js?id=UA-23581568-13"></script>
        <script>
            window.dataLayer = window.dataLayer || [];
            function gtag() {
                dataLayer.push(arguments);
            }
            gtag("js", new Date());

            gtag("config", "UA-23581568-13");
        </script>
        
    </body>

    <!-- Mirrored from preview.colorlib.com/theme/docmed/ by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 20 Sep 2021 16:38:43 GMT -->
</html>
